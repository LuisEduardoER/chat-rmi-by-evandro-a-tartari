...
public class UIUtil {
            ...
            if (tracker.isErrorAny()) {
                /* DEBUG */System.err
                        .println(MessageFormat
                                .format(
                                        Messages
                                                .getString("UIUtil.imageDidNotLoad"), new Object[] { image.toString() })); //$NON-NLS-1$
            }
        }
    ...
    /**
     * Load an image using the specified <code>Class</code>es
     * <code>ClassLoader</code> and the given resource name. <code>null</code>
     * will be returned if the image could not be loaded.
     * 
     * @param clazz
     *            class to get class loader from
     * @param resource
     *            resource name
     */
    @SuppressWarnings("unchecked")
    public static Image loadImage(Class clazz, String resource) {
        ...
        /*
         * String path = null; if (resource.startsWith("/")) { path = resource;
         * } else { int idx = clazz.getName().lastIndexOf('.'); String
         * packageName = idx == -1 ? null : clazz.getName().substring(0, idx);
         * path = packageName == null ? "" : "/" + packageName; path =
         * path.replace('.', '/'); path += ("/" + resource); }
         */
        ...
        if (img == null) {
            System.err
                    .println(MessageFormat
                            .format(
                                    Messages
                                            .getString("UIUtil.couldNotLocateImage"), new Object[] { resource })); //$NON-NLS-1$
        }
        return img;
    }
    ...
    public static void gridBagAdd(Container parent, Component componentToAdd,
        ...
        if (!(parent.getLayout() instanceof GridBagLayout)) {
            throw new IllegalArgumentException(Messages
                    .getString("UIUtil.parentMustHaveGridBagLayout")); //$NON-NLS-1$
        }
     ...
     * 
     * @param p
     *            postion from <code>SwingConstants</code>
     * @param c
     *            component
     */
    public static void positionComponent(int p, Component c) {
        ...
        try {
             ...
             */
            // #else
            d = c.getGraphicsConfiguration().getDevice()
        ...
        if (d == null) {
            ...
            d = new Rectangle(0, 0, s != null ? s.width : 800,
                    s != null ? s.height : 600);
            // TODO Find a better way of taking care of centering on dual-head
            // displays
     ...
     * 
     * @param id
     *            stock image id. See {@link com.sshtools.ui.StockIcons} for
     *            constants.
     * @param clazz
     *            the {@link Class} to derive a {@link ClassLoader} from to load
     *            the image with.
     */
    @SuppressWarnings("unchecked")
     ...
     *            constants.
     * @param resource
     *            the resource path to use to load the image.
     */
    @SuppressWarnings("unchecked")
